...
class PySee:
    ...
    def init_config(self):
        ...
        print(
            'PySee\'s configuration file will be stored at "~/.config/pysee/pysee.conf".\n'
        )
        ...
        try:
            ...
            values["IMGUR_CLIENT_ID"] = input(
                "Imgur API Client ID (http://api.imgur.com/oauth2/addclient): ('') "
            )
        ...
        if upload:
            ...
            logger.info(
                'Image upload: "{}" was uploaded to "{}" at "{}".'.format(
                    os.path.split(image_filepath)[-1], host_name, image_url
                )
            )
        else:
