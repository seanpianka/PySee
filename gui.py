...
def popupmsg(msg):
    ...
    label = ttk.Label(
        ...
        text=msg,
        font=MEDIUM_FONT,
    ).pack(side="top", fill="both", pady=20, padx=35, expand=True)
    ttk.Button(container, text="Close", command=popup.destroy).pack(
        side="bottom", pady=(0, 15)
    )
...
class PySeeApp(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        capturemenu.add_command(
            label="Fullscreen", command=lambda: self.capture("full", *(values.values))
        )
        capturemenu.add_command(
            label="Region", command=lambda: self.capture("region", *(values.values()))
        )
        capturemenu.add_command(
            label="Window", command=lambda: self.capture("window", *(values.values()))
        )
        menubar.add_cascade(label="Capture", menu=capturemenu)

        settingsmenu = tk.Menu(menubar, tearoff=0)
        settingsmenu.add_command(
            label="Capture Settings", command=lambda: self.show_frame(SettingsPage)
        )
        settingsmenu.add_command(
        ...
        aboutmenu.add_command(label="About", command=lambda: popupmsg(ABOUT_MSG))
        ...
        for f in (CapturePage, SettingsPage, ConfigurationPage):
            ...
            frame.grid(
                row=0, column=0, sticky="nsew", columnspan=4, rowspan=self.MAX_ROW
            )
        ...
        for page, title in self.frames.items():
            ...
            ttk.Button(
                ...
                text=title[1],
            ).grid(row=self.MAX_ROW, column=i, sticky="ew")
            i += 1
    ...
    def __init__(self, parent, controller):
        ...
        class Column(tk.Frame):
            def __init__(self, parent, title, num_rows):
                ...
                tk.Label(
                    self.title, bg="#121212", fg="#f7f7f7", font=MEDIUM_FONT, text=title
                ).pack(pady=TITLE_PADDING)
        ...
        self.top_frame = tk.Frame(
            self, width=WINDOW_WIDTH, height=(WINDOW_HEIGHT / 3) * 2, bg="#181818"
        )
        for i in range(1):
            self.top_frame.rowconfigure(i, weight=1)
        for i in range(3):
            self.top_frame.columnconfigure(i, weight=1)

        # Image Host, column 1
        self.col_image_host = Column(
            self.top_frame, "Image Host", len(ImageHost.valid_hosts)
        )

        image_host = tk.StringVar()
        for i, (host, host_upload) in enumerate(ImageHost.valid_hosts.items()):
            ttk.Radiobutton(
                self.col_image_host.content, text=host, value=host, var=image_host
            ).grid(row=i, column=0, pady=10, padx=30, sticky="ew")
        image_host.set(DEFAULTS["HOST"])

        self.col_image_host.place(0)

        # Capture Tool, column 2
        self.col_capture_tool = Column(
            self.top_frame, "Capture Tool", len(CaptureTool.valid_tools)
        )

        capture_tool = tk.StringVar()
        for i, (tool, tool_capture) in enumerate(CaptureTool.valid_tools.items()):
            ttk.Radiobutton(
                self.col_capture_tool.content, text=tool, value=tool, var=capture_tool
            ).grid(row=i, column=0, pady=10, padx=30, sticky="ew")
        ...
        ttk.Checkbutton(
            self.col_after_capture.content,
            text="Copy to Clipboard",
            var=clipboard_option,
        ).grid(row=0, column=0, pady=10, padx=30, sticky="ew")

        upload_option = tk.BooleanVar(None, DEFAULTS["UPLOAD"])
        ttk.Checkbutton(
            self.col_after_capture.content,
            text="Upload to Image Host",
            var=upload_option,
        ).grid(row=1, column=0, pady=10, padx=30, sticky="ew")

        logging_option = tk.BooleanVar(None, DEFAULTS["LOGGING"])
        ttk.Checkbutton(
            self.col_after_capture.content,
            text="Logging to Terminal",
            var=logging_option,
        ).grid(row=2, column=0, pady=10, padx=30, sticky="ew")

        save_option = tk.BooleanVar(None, DEFAULTS["SAVE"])
        ttk.Checkbutton(
            self.col_after_capture.content, text="Save After Capture", var=save_option
        ).grid(row=3, column=0, pady=10, padx=30, sticky="ew")

        self.col_after_capture.place(2)

        ## Bottom frame for "mode"/action buttons
        self.bottom_frame = tk.Frame(
            self, width=WINDOW_WIDTH, height=WINDOW_HEIGHT / 3, bg="#121212"
        )
        for i in range(1):
        ...
        ttk.Button(
            self.bottom_frame,
            text="Region",
            command=lambda: controller.capture(
                "region",
                image_host.get(),
                capture_tool.get(),
...
class ConfigurationPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button = ttk.Button(self, text="Save Changes", command=lambda: save_changes())
...
class SettingsPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button = ttk.Button(
            ...
            command=lambda: save_preferences(
                controller,
                image_host.get(),
                capture_tool.get(),
                clipboard_option.get(),
...
def main():
    ...
    try:
        keyboard.add_hotkey(
            "ctrl+shift+2", lambda: app.capture(mode="full", *hotkey_values)
        )
        keyboard.add_hotkey(
            "ctrl+shift+3", lambda: app.capture(mode="window", *hotkey_values)
        )
        keyboard.add_hotkey(
            "ctrl+shift+4", lambda: app.capture(mode="region", *hotkey_values)
        )
    except ImportError:
        app.bind("<Control-Shift-2>", lambda: app.capture(mode="full", *hotkey_values))
        app.bind(
            "<Control-Shift-3>", lambda: app.capture(mode="window", *hotkey_values)
        )
        app.bind(
            "<Control-Shift-4>", lambda: app.capture(mode="region", *hotkey_values)
        )
        print(
            "GUI not started as root, unable to listen globally for capture hotkeys..."
        )
